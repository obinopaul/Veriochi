...
- `apps/space` and `apps/live` were removed during the public sharing and realtime de-scoping. References to their Dockerfiles or scripts are historical only.
- `apps/space` and `apps/live` were removed during the public sharing and realtime de-scoping. References to their Dockerfiles or scripts are historical only.
...
- Ports: `apps/web` (3000) and `apps/admin` (3001) are typical defaults in docker-compose.
